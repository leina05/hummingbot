...
from hummingbot.core.event.events import OrderBookTradeEvent, TradeType, OrderBookEvent
import asyncio
import logging
import unittest
...
from hummingbot.connector.exchange.gemini.gemini_order_book_tracker import GeminiOrderBookTracker
...
class GeminiOrderBookTrackerUnitTest(unittest.TestCase):
    order_book_tracker: Optional[GeminiOrderBookTracker] = None
    ...
    def setUpClass(cls):
        ...
        cls.order_book_tracker: GeminiOrderBookTracker = GeminiOrderBookTracker(trading_pairs=cls.trading_pairs)
    ...
    def test_tracker_integrity(self):
        ...
        self.assertGreaterEqual(
            test_order_book.get_price_for_volume(True, 10).result_price, test_order_book.get_price(True)
        )
        self.assertLessEqual(
            test_order_book.get_price_for_volume(False, 10).result_price, test_order_book.get_price(False)
        )
    ...
    def test_diff_msg_get_added_to_order_book(self):
        ...
        open_message = GeminiOrderBook.diff_message_from_exchange(raw_open_message)
        ...
        change_message = GeminiOrderBook.diff_message_from_exchange(raw_change_message)
        ...
        match_message = GeminiOrderBook.diff_message_from_exchange(raw_match_message)
        ...
        done_message = GeminiOrderBook.diff_message_from_exchange(raw_done_message)
    ...
    def test_api_get_last_traded_prices(self):
        prices = self.ev_loop.run_until_complete(
            GeminiAPIOrderBookDataSource.get_last_traded_prices(["BTC-USD", "LTC-USD"])
        )
