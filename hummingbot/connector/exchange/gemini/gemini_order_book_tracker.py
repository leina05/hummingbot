...
from hummingbot.connector.exchange.gemini.gemini_api_order_book_data_source import (
    GeminiAPIOrderBookDataSource,
)
from hummingbot.connector.exchange.gemini.gemini_order_book_message import GeminiOrderBookMessage
...
class GeminiOrderBookTracker(OrderBookTracker):
    ...
    def __init__(self, trading_pairs: Optional[List[str]] = None):
        super().__init__(
            data_source=GeminiAPIOrderBookDataSource(trading_pairs=trading_pairs), trading_pairs=trading_pairs
        )
        ...
        self._order_books: Dict[str, GeminiOrderBook] = {}
        self._saved_message_queues: Dict[str, Deque[GeminiOrderBookMessage]] = defaultdict(lambda: deque(maxlen=1000))
        self._active_order_trackers: Dict[str, GeminiActiveOrderTracker] = defaultdict(GeminiActiveOrderTracker)
    ...
    async def _order_book_diff_router(self):
        ...
        while True:
            try:
                ob_message: GeminiOrderBookMessage = await self._order_book_diff_stream.get()
                ...
                order_book: GeminiOrderBook = self._order_books[trading_pair]
                ...
                if ob_message.content["type"] == "match":  # put match messages to trade queue
                    trade_type = (
                        ...
                        if ob_message.content["side"].upper() == "SELL"
    ...
    async def _track_single_book(self, trading_pair: str):
        ...
        """
        past_diffs_window: Deque[GeminiOrderBookMessage] = deque()
        self._past_diffs_windows[trading_pair] = past_diffs_window

        message_queue: asyncio.Queue = self._tracking_message_queues[trading_pair]
        order_book: GeminiOrderBook = self._order_books[trading_pair]
        active_order_tracker: GeminiActiveOrderTracker = self._active_order_trackers[trading_pair]
        ...
        while True:
            try:
                message: GeminiOrderBookMessage = None
                saved_messages: Deque[GeminiOrderBookMessage] = self._saved_message_queues[trading_pair]
                ...
                if message.type is OrderBookMessageType.DIFF:
                    ...
                    past_diffs: List[GeminiOrderBookMessage] = list(past_diffs_window)
